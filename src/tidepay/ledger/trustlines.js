...
function getAccountLines(api, address, ledgerVersion, options, marker, limit) {
  var request = {
    account: address,
  ...
  return api.doAccountLines(request).then(_.partial(formatResponse, options));
}
...
function getTrustlines(address) {
  ...
  return this.getLedgerVersion().then(function (ledgerVersion) {
    var getter = _.partial(getAccountLines, _this, address, options.ledgerVersion || ledgerVersion, options);
