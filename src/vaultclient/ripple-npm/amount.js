...
Amount.prototype.not_equals_why = function (d, ignore_issuer) {
  ...
  var type = this._is_native ? 'XTP' : 'Non-XTP';
  ...
  if (!this._is_native) {
    if (!this._currency.equals(d._currency)) {
      return 'Non-XTP currency differs.';
    }
    if (!ignore_issuer && !this._issuer.equals(d._issuer)) {
      return 'Non-XTP issuer differs: ' + d._issuer.to_json() + '/' + this._issuer.to_json();
    }
  }
};
