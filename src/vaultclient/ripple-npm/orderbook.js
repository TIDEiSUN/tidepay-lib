...
function OrderBook(remote, currencyGets, issuerGets, currencyPays, issuerPays, key) {
  ...
  this.sortOffers = this._currencyGets.has_interest() ? _sortOffers.bind(this) : _sortOffersQuick;
...
OrderBook.prototype.requestTransferRate = function (callback) {
  ...
  function handleAccountInfo(err, info) {
    ...
    self._issuerTransferRate = Amount.from_json(info.account_data.TransferRate || OrderBook.DEFAULT_TRANSFER_RATE);
...
OrderBook.prototype.applyTransferRate = function (balance) {
  ...
  var adjustedBalance = OrderBookUtils.normalizeAmount(balance).divide(this._issuerTransferRate).multiply(OrderBook.DEFAULT_TRANSFER_RATE).to_json().value;
...
OrderBook.prototype.setOfferFundedAmount = function (offer) {
  ...
  var takerGets = Amount.from_json(offer.TakerGets);
  ...
  offer.is_fully_funded = fundedAmount.is_comparable(currentOfferSum) && fundedAmount.compareTo(currentOfferSum) >= 0;
...
OrderBook.prototype.onTransaction = function (transaction) {
  ...
  if (--this._transactionsLeft === 0 && !this._waitingForOffers) {
    var lastClosedLedger = this._remote.getLedgerSequence() - 1;
    if (this._isAutobridgeable) {
      if (this._canRunAutobridgeCalc()) {
        if (this._legOneBook._offersModifiedAt === lastClosedLedger || this._legTwoBook._offersModifiedAt === lastClosedLedger) {
...
OrderBook.prototype.onLedgerClosed = function (message) {
  if (!message || message && !_.isNumber(message.txn_count) || !this._subscribed || this._destroyed || this._waitingForOffers) {
    return;
  }
    ...
    switch (node.nodeType) {
      case 'DeletedNode':
        if (self._validAccounts[node.fields.Account] === undefined) {
          assert(UInt160.is_valid(node.fields.Account), 'node has an invalid account');
...
OrderBook.prototype.computeAutobridgedOffers = function () {
  ...
  autobridgeCalculator.calculate(function (autobridgedOffers) {
    _this._offersAutobridged = autobridgedOffers;
    callback();
  });
};
...
OrderBook.prototype.computeAutobridgedOffersWrapper = function () {
  ...
  if (!this._gotOffersFromLegOne || !this._gotOffersFromLegTwo || !this._synchronized || this._destroyed || this._calculatorRunning) {
    return;
  }

  this._calculatorRunning = true;
  var startTime = Date.now();
  this.computeAutobridgedOffers(function () {
    _this2.mergeDirectAndAutobridgedBooks();
    _this2._calculatorRunning = false;
  });
};

function _sortOffers(a, b) {
  var aQuality = OrderBookUtils.getOfferQuality(a, this._currencyGets);
  var bQuality = OrderBookUtils.getOfferQuality(b, this._currencyGets);
...
OrderBook.prototype.mergeDirectAndAutobridgedBooks = function () {
  ...
  if (_.isEmpty(this._offers) && _.isEmpty(this._offersAutobridged)) {
    if (this._synchronized && this._gotOffersFromLegOne && this._gotOffersFromLegTwo) {
  ...
  this._mergedOffers = this._offers.concat(this._offersAutobridged).sort(this.sortOffers);
